var socket = io.connect('81.166.132.210:2520', {secure: false}); //This line declares a socket.io object to var "socket" and connects to the server (change the IP-address and port to your own)
//var socket = io.connect('192.168.10.120:2520', {secure: false}); //This line declares a socket.io object to var "socket" and connects to the server (change the IP-address and port to your own)
...
function registerUser(key, username, password) {
    ...
    socket.once('registration_success',function(username) { //If the registration is successfull
        console.log('Brukeren din ble registrert!');
        title.innerHTML = `Brukeren din ${username} ble registrert!`;
    });

    socket.once('registration_denied',function() { //If the registration failed
